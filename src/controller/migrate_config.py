...
def main() -> None:
    ...
    if args.old_install_dir:
        ...
        print(f"  ./bin/profile_mgr.sh --migrate-old {args.old_install_dir} <target-profile>")
        print(f"  .\\bin\\profile_mgr.ps1 --migrate-old {args.old_install_dir} <target-profile>")
    elif args.export:
        print("Use profile_mgr instead:")
        print(f"  ./bin/profile_mgr.sh --export <profile-name> {args.export}")
        print(f"  .\\bin\\profile_mgr.ps1 --export <profile-name> {args.export}")
    elif args.import_resources:
        print("Use profile_mgr instead:")
        print(f"  ./bin/profile_mgr.sh --import-profile {args.import_resources}")
        print(f"  .\\bin\\profile_mgr.ps1 --import-profile {args.import_resources}")
    raise SystemExit(1)
