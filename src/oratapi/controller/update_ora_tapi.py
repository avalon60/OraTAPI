...
from oratapi.lib.app_utils import get_latest_version, get_latest_dist_url, download_file
import platform
...
def upgrade_files(upgrade_dir: Path) -> None:
    ...
    # Upgrade config sample files. Prefer the simplified config root layout, but
    # continue to accept older archives that still place samples under config/samples.
    upgrade_config_dir = upgrade_resources / "config"
    config_sample_roots = [upgrade_config_dir]
    legacy_config_samples_dir = upgrade_config_dir / "samples"
    if legacy_config_samples_dir.exists():
        config_sample_roots.append(legacy_config_samples_dir)

    seen_sample_names: set[str] = set()
    for sample_root in config_sample_roots:
        for sample_file in sample_root.rglob("*.sample"):
            if sample_file.name in seen_sample_names:
                continue
            seen_sample_names.add(sample_file.name)
            target_file = current_resources / "config" / sample_file.name
    ...
    sample_config_pathname = upgrade_resources / "config" / "OraTAPI.ini.sample"
    if not sample_config_pathname.exists():
        sample_config_pathname = upgrade_resources / "config" / "samples" / "OraTAPI.ini.sample"
